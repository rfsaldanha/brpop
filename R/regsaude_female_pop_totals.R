...
regsaude_female_pop_totals <- function(type = "standard", source = "bmh"){
  # Assertions
  checkmate::assert_choice(x = type, choices = c("standard", "reg_saude_449"))
  checkmate::assert_choice(x = source, choices = c("bmh", "ufrn"))

  if(type == "standard"){
    res <- regsaude_female_pop(source = source)
  } else if(type == "reg_saude_449"){
    res <- regsaude_female_pop(type = "reg_saude_449", source = source)
  }
